...
module.exports = {
    ...
    run: async(client, message, args) => {
        ...
        let sesteolmayan = message.guild.members.cache.filter(s => s.roles.cache.has('880923348522127438')).filter(s => !s.voice.channel).map(s => s).join(' ')
        let sesteolan = message.guild.members.cache.filter(s => s.roles.cache.has('880923348522127438')).filter(s => s.voice.channel).map(s => s).join(', ')
