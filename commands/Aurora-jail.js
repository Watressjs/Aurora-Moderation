...
module.exports = {
    ...
    run: async(client, message, args) => {
        ...
        message.channel.send(`<:Cezalandrld:886168653626761228> ${member} Adlı Kullanıcı **${reason}** Sebebi ile **${time}** süresince cezalıya atıldı!`).then(x => x.delete({ timeout: 5000 })).then(message.react(client.config.tik));
        ...
        channel.send(embed.setTitle('').setDescription(`Yetkili: ${message.author} - (\`${message.author.id}\`) \n Jaile Atılan: ${member} - \`${member.id}\` \n Sebep: ${reason} \n Süre: ${time} \n Tarih: **${moment().format('LLL')}**`))
        if (time) setTimeout(() => {
